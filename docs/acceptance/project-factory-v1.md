...
| `npm view codex-project-factory version` | Registry package not yet published; GitHub source install is the current npm entry |
...
npm registry 尚未发布此包；当前仓库已验证可通过 `npm install github:Changxin-YR/Project-workflow` 安装源码，或通过 `npm pack` 生成干净的分发 tarball。安装后将 `node_modules/codex-project-factory` 注册为 Codex marketplace，再执行 `codex plugin add codex-project-factory@project-workflow`。发布者完成 npm 登录和发布后，再切换到 registry 安装。
